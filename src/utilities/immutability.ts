export class Immutability {
	...
	static freeze<T>(obj: T, excluded?: Set<any>): Readonly<T> {
		if (excluded && excluded.has(obj)) return obj
		...
		if (Object(obj) === "object") {
			...
			Object.getOwnPropertyNames(obj).forEach((prop) =>
				Immutability.freeze(obj[<keyof T>prop], childrenExcluded)
			)
		}
	...
	static unfreeze<T>(obj: Readonly<T>): T {
		...
		Object.assign(unfrozenObj as any, obj)

		Object.getOwnPropertyNames(obj).forEach((prop) => {
			unfrozenObj[<keyof T>prop] = Immutability.unfreeze(
				unfrozenObj[<keyof T>prop]
			)
		})
